...
import os
import pandas as pd
from CONSTANTES import DICCIONARIO_ETNIA, DICCIONARIO_GENERO, DICCIONARIO_PELO, OPCIONES_GENERO, OPCIONES_PIEL, OPCIONES_PELO, \
    COMBOBOX_HEIGHT, COMBOBOX_WIDTH, SCREEN_WIDTH, SCREEN_HEIGHT, N_IMGS, CARPETA_DESTINO, LUGAR_ALMACENAMIENTO, \
    directorio_raiz, IMG_WIDTH, IMG_HEIGHT, MARGEN_VERTICAL
...
def seleccionador_genero_click(event):
    """
    Muestra aquellas imágenes que se corresponden con la elección del usuario entre (mujer u hombre)
    :param event:
    :return:
    """
    global lista_imgs_labels
...
def seleccionador_piel_click(event):
    """
    Muestra aquellas imágenes que se corresponden con la elección del usuario entre (negro, moreno o blanco)
    :param event:
    :return:
    """
...
def seleccionador_pelo_click(event):
    """
        Muestra aquellas imágenes que se corresponden con la elección del usuario entre (corto, medio o largo)
        :param event:
        :return:
    """
...
def clasificar():
    """
    Elimina de la pantalla todos los widgets que no se correspondan con la pantalla de clasificar y muestra aquellos que sí
    :return:
    """
    ### BORRAR WIDGETS ###
    global lista_imgs_labels
...
def abrir_carpeta_origen():
    """
    Permite al usuario elegir la carpeta de origen de sus imágenes
    :return:
    """
    global carpeta_origen
...
def ejecutar():
    """
    Realiza la prediccion utilizando la función 'predecir' de functions y
    muestra el número de imágnes de cada clase que se han clasificado
    :return:
    """
    global lista_imgs
    global data_frame
    try:
        ...
        n_total_clasificadas.configure(text="Se han clasificado " + str(resultados[0] + resultados[1]) + " imágenes")
...
def filtrar():
    """
    Elimina de la pantalla todos los widgets que no se correspondan con la pantalla de filtrar y muestra aquellos que sí
    :return:
    """
    global lista_imgs_labels
    global lista_imgs
...
def click_mouse(evento):
    """
    Muestra el nombre de la imagen sobre la que el ratón está parado encima cuando hace click izquierdo
    :param evento:
    :return:
    """
    global page
